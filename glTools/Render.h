...
auto pos2 = glm::vec3(4.39, 0.21, 5.14);
auto front2 = glm::vec3(-1.0, 0.037, -0.08);
auto zoom2 = 25.0f;

auto pos3 = glm::vec3(2.95, 2.57, 7.18);
auto front3 = glm::vec3(-0.67, -0.52, -0.53);
auto zoom3 = 25.0f;
...
public:
	...
	void GLTOOLSDLL_API renderSingleSphere(const Shader& shader, const glm::vec3& pos, const glm::vec3& scale,
									 const glm::vec4& color = glm::vec4(0.0f));

	void GLTOOLSDLL_API renderTours(const Shader& shader);
	//所有的工程都使用这个函数来计算误差率，免得每个工程都去改一遍代码
	void GLTOOLSDLL_API testErrorRateScene(const Shader& shader, scene_mode vMode);
	//论文中使用的误差率测试场景
	void GLTOOLSDLL_API FinalErrorRateScene(const Shader& shader);
...
private:
	...
	void __renderTours1(float radius, float cs_radius);
	void __renderTours2(float radius, float cs_radius);
	void __initToursVAO(int sides, int cs_sides, float radius, float cs_radius, bool initVAO1 = true);//圆环
	...
	unsigned int toursVAO1 = 0;
	unsigned int toursVAO2 = 0;
